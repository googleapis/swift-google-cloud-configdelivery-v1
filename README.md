...
```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-configdelivery-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudConfigDeliveryV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudConfigDeliveryV1 <target-name> --package swift-google-cloud-configdelivery-v1
```
